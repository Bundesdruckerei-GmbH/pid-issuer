...
public record CredentialDto(String format, @JsonProperty("doctype") String docType, @JsonProperty("vct") String sdJwt) {
    ...
    public void validate(String authority) {
        ...
        switch (optionalFormat.get()) {
            case SD_JWT -> validateType(authority, SupportedFormats.SD_JWT, "vct", sdJwt);
            case SD_JWT_AUTH_CHANNEL -> validateType(authority, SupportedFormats.SD_JWT_AUTH_CHANNEL, "vct", sdJwt);
    ...
    private void validateType(String authority, SupportedFormats supportedFormat, String typeName, String typeValue) {
        if (StringUtils.isBlank(typeValue)) {
            throw new InvalidCredentialRequestException(typeName + " parameter is missing");
        } else if (!(authority + supportedFormat.getTypeValue()).equals(typeValue)) {
    ...
    public enum SupportedFormats {
        SD_JWT(SdJwtVcCredentialFormat.INSTANCE.getFormat(), "credentials/pid/1.0"),
        SD_JWT_AUTH_CHANNEL(SdJwtVcAuthChannelCredentialFormat.INSTANCE.getFormat(), "credentials/pid/1.0"),
