...
public class KeyProofHandler implements OidHandler {
    ...
    private final int maxProofsSize;

    public KeyProofHandler(KeyProofService keyProofService, List<Class<? extends CredentialRequest>> requestsUsingProof) {
        this(keyProofService, requestsUsingProof, 0);
    }
    ...
    private void checkProofs(List<Proof> proofs, Proof proof) {
        if (maxProofsSize == 0 && !proofs.isEmpty()) {
            throw new InvalidCredentialRequestException("No proofs expected");
        }
        if (proofs.size() > maxProofsSize) {
